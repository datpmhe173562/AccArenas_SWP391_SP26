...
function convertCSharpToTypeScript(csharpType) {
  ...
  if (csharpType.includes('List<') || csharpType.includes('IList<')) {
    ...
    const tsInnerType = typeMapping[innerType] || innerType.toLowerCase();
    return `${tsInnerType}[]`;
  }
  
  return typeMapping[csharpType] || csharpType.toLowerCase();
}
...
function parseCSharpFile(filePath) {
  ...
  // Regex to match class definitions and their properties
